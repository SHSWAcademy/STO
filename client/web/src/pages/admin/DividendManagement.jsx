...
export function DividendManagement() {
      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsViewingDetail(false)} className="p-2 rounded-md bg-white border border-[#e0dace] text-[#9a9080] hover:text-[#2a2820] transition-colors">
              <ArrowRight className="rotate-180 w-5 h-5" />
            </button>
            <div>
              <h2 className="text-xl font-semibold text-[#2a2820]">배당 스케줄 및 내역 관리</h2>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 bg-white border border-[#e0dace] rounded-lg p-8 space-y-6">
            ...
            <div className="space-y-4">
              {[['배당 기준일 (고정)', fixedBaseDate, 'date'], ['지급 예정일 (고정)', fixedPaymentDate, 'date']].map(([label, value]) => (
                <div key={label} className="space-y-1.5">
                  ...
                  <input type="date" value={value} readOnly
                    className="w-full bg-[#e0dace] border border-[#e0dace] rounded-md px-4 py-3 text-sm text-[#9a9080] outline-none cursor-not-allowed font-medium"
                  />
                </div>
              ))}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-[#9a9080] uppercase tracking-widest ml-1">월 수익 (KRW)</label>
                  <input type="number" placeholder="0" value={form.monthlyProfit}
                    ...
                    className="w-full bg-[#f7f5f0] border border-[#e0dace] rounded-md px-4 py-3 text-sm text-[#2a2820] outline-none focus:border-[#b85450] transition-colors font-medium"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-semibold text-[#9a9080] uppercase tracking-widest ml-1">1주당 배당금</label>
                  <input type="number" readOnly value={form.amountPerToken}
                    className="w-full bg-[#f7f5f0]/50 border border-[#e0dace] rounded-md px-4 py-3 text-sm text-[#9a9080] outline-none font-mono font-medium"
                  />
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] font-semibold text-[#9a9080] uppercase tracking-widest ml-1">배당 세부 내역 (PDF URL)</label>
                <div className="relative">
                  <input type="text" placeholder="https://example.com/dividend.pdf" value={form.pdfUrl}
                    onChange={e => setForm({ ...form, pdfUrl: e.target.value })}
                    className="w-full bg-[#f7f5f0] border border-[#e0dace] rounded-md px-4 py-3 text-sm text-[#2a2820] outline-none focus:border-[#b85450] transition-colors font-medium pr-10"
                  />
              ...
              <button onClick={handleRegister}
                className="w-full py-4 bg-[#b85450] text-white text-xs font-semibold uppercase tracking-widest rounded-md hover:bg-[#a04040] transition-colors mt-4">
                스케줄 및 내역 등록하기
              </button>
            </div>
          </div>

          <div className="lg:col-span-2 bg-white border border-[#e0dace] rounded-lg p-8 space-y-6">
            <div className="flex items-center justify-between border-b border-[#e0dace] pb-4">
              <h3 className="text-sm font-semibold text-[#2a2820] uppercase tracking-widest">배당 지급 및 스케줄 이력</h3>
              <div className="flex gap-2">
                <span className="px-2 py-0.5 rounded bg-[#e0f0e8] text-[#4a7a60] text-[9px] font-semibold uppercase">지급완료</span>
                <span className="px-2 py-0.5 rounded bg-[#ffe65a]/30 text-[#c9a84c] text-[9px] font-semibold uppercase">예정</span>
              </div>
            </div>
                  ...
                  {MOCK_DIVIDEND_HISTORY.filter(h => h.assetName === selectedAsset.name).map(h => (
                    <tr key={h.id} className="hover:bg-[#f7f5f0] transition-colors">
                      ...
                      <td className="py-4 text-center">
                        <button className="p-2 text-[#b85450] hover:bg-[#fde8e8] rounded-lg transition-colors"><FileText size={16} /></button>
                      </td>
                      <td className="py-4 text-center">
                        <span className="px-2 py-0.5 rounded bg-[#e0f0e8] text-[#4a7a60] text-[9px] font-semibold uppercase">완료</span>
                      </td>
                    </tr>
                  ))}
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-[#2a2820]">배당금 관리</h1>
          <p className="text-sm text-[#9a9080]">STO 자산별 배당금 정산 및 지급 내역을 관리합니다.</p>
        </div>
        <div className="flex items-center gap-5 bg-white border border-[#e0dace] rounded-lg px-6 py-3">
          <div className="flex flex-col items-center justify-center bg-[#b85450] text-white rounded-md w-14 h-14">
          ...
          <div>
            <h3 className="text-lg font-semibold text-[#2a2820] leading-tight">정산 대상 월</h3>
      ...
      <div className="flex items-center gap-4 p-2 bg-[#f7f5f0] border border-[#e0dace] rounded-lg w-fit">
        {[
          ...
          { id: 'unregistered', label: '당월 미등록',    value: TOKENS.length - registeredAssets.size, icon: AlertCircle,  color: 'text-[#b85450]', bg: 'bg-[#fde8e8]' },
        ].map(stat => (
      ...
      <div className="bg-white rounded-lg border border-[#e0dace] overflow-hidden">
        <div className="p-6 border-b border-[#e0dace] flex items-center justify-between bg-[#f7f5f0]">
          <div className="flex items-center gap-3">
            <h3 className="text-lg font-semibold text-[#2a2820]">배당 및 정산 현황</h3>
            <div className="px-2 py-0.5 rounded bg-[#fde8e8] text-[#b04040] text-[10px] font-semibold">
            ...
            <thead>
              <tr className="bg-[#f7f5f0] border-b border-[#e0dace]">
                {['자산 정보','등록 상태','월 수익 (KRW)','누적 잔여금','관리'].map(h => (
                  <th key={h} className={`px-6 py-4 text-[10px] font-semibold text-[#9a9080] uppercase tracking-wide ${h === '월 수익 (KRW)' || h === '누적 잔여금' ? 'text-right' : h === '등록 상태' || h === '관리' ? 'text-center' : ''}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-[#e0dace]">
                    ...
                    <td className="px-6 py-4 text-center">
                      {isRegistered ? (
                        ...
                        <div className={cn(
                          ...
                          isUrgent ? 'bg-[#fde8e8] text-[#b04040]' : 'bg-[#f7f5f0] text-[#9a9080]'
                        )}>
