  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-[#2a2820]">공시/공지 관리</h1>
          <p className="text-sm font-bold text-[#9a9080]">플랫폼 공지사항 및 자산별 공시 내역을 관리합니다.</p>
        </div>
        <button
          onClick={() => activeTab === 'notices' ? setIsAddingNotice(true) : setIsAddingDisclosure(true)}
          className="flex items-center gap-2 px-6 py-3 bg-[#4a72a0] text-white text-sm font-bold rounded-xl hover:opacity-90 transition-all shadow-lg shadow-[#4a72a0]/20"
        >
              ...
              <thead>
                <tr className="bg-[#f7f5f0] border-b border-[#e0dace]">
                  {['제목','카테고리','작성일','중요','관리'].map(h => (
                    <th key={h} className={`px-6 py-4 text-[10px] font-black text-[#9a9080] uppercase tracking-wider ${h === '중요' || h === '관리' ? 'text-center' : ''}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-[#e0dace]">
                {filteredNotices.map(n => (
                  <tr key={n.id} className="hover:bg-[#f7f5f0] transition-all group">
                    <td className="px-6 py-4"><p className="text-sm font-black text-[#2a2820]">{n.title}</p></td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                        ...
                        <button className="p-2 text-[#b85450] hover:bg-[#4a3232] rounded-lg transition-all"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4 p-2 bg-[#f7f5f0] border border-[#e0dace] rounded-2xl w-fit">
            {[
              ...
              { id: 'pending',  label: '검토 대기',        value: disclosures.filter(d => d.status === '검토대기').length, icon: AlertCircle, color: 'text-[#b85450]', bg: 'bg-[#4a3232]' },
            ].map(stat => (
              <button key={stat.id} onClick={() => setDisclosureFilter(stat.id)}
                className={cn('flex items-center gap-4 px-8 py-4 rounded-2xl transition-all text-left min-w-[200px]',
                  disclosureFilter === stat.id ? 'bg-white shadow-md border border-[#e0dace]' : 'hover:bg-white/50 border border-transparent'
                )}>
                ...
                <div>
                  ...
                  <h3 className="text-xl font-black text-[#2a2820]">{stat.value}건</h3>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-[#e0dace] shadow-sm overflow-hidden">
            <div className="px-8 pt-6 pb-0 border-b border-[#e0dace] bg-[#f0ede4] space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button className="p-3 bg-white border border-[#e0dace] rounded-2xl text-[#7a7060] hover:bg-[#f7f5f0] shadow-sm transition-all">
              ...
              <div className="flex gap-1">
                {['전체', '배당', '일반'].map(tab => (
                  <button key={tab} onClick={() => setDisclosureTypeTab(tab)}
                    className={cn('px-5 py-2.5 text-sm font-black transition-all border-b-2 -mb-px',
                ...
                <thead>
                  <tr className="bg-[#f7f5f0] border-b border-[#e0dace]">
                    {['종목','유형','제목','일자','파일','상태','관리'].map(h => (
                      <th key={h} className={`px-6 py-4 text-[10px] font-black text-[#9a9080] uppercase tracking-wider ${['파일','상태','관리'].includes(h) ? 'text-center' : ''}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#e0dace]">
                  {filteredDisclosures.map(item => (
                    <tr key={item.id} className="hover:bg-[#f7f5f0] transition-all group">
                      <td className="px-6 py-4 text-sm font-black text-[#2a2820]">{item.asset}</td>
                      <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-[#e0dace] flex items-center justify-between bg-[#f7f5f0]">
              <h3 className="text-lg font-black text-[#2a2820]">공시 정보 수정</h3>
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#9a9080] uppercase tracking-widest">상태</label>
            ...
            <div className="p-6 bg-[#f7f5f0] border-t border-[#e0dace] flex gap-3">
              <button onClick={() => setEditingDisclosure(null)} className="flex-1 py-3 rounded-xl bg-white border border-[#e0dace] text-[#9a9080] text-sm font-bold hover:bg-[#e0dace] transition-all">취소</button>
              <button onClick={handleSaveDisclosure} className="flex-[2] py-3 rounded-xl bg-[#4a72a0] text-white text-sm font-bold hover:opacity-90 shadow-lg shadow-[#4a72a0]/20 transition-all">저장하기</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isAddingNotice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-[#e0dace] flex items-center justify-between bg-[#f7f5f0]">
              <h3 className="text-lg font-black text-[#2a2820]">신규 공지사항 등록</h3>
            ...
            <div className="p-8 space-y-6">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#9a9080] uppercase tracking-widest">공지 제목</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#9a9080] uppercase tracking-widest">카테고리</label>
                <div className="flex gap-2 p-1 bg-[#e0dace] rounded-xl">
                  {['일반','시스템'].map(cat => (
                    <button key={cat} type="button" onClick={() => setNoticeForm({ ...noticeForm, category: cat })}
                      className={cn('flex-1 py-2.5 rounded-lg text-sm font-black transition-all',
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#9a9080] uppercase tracking-widest">중요 공지 여부</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#9a9080] uppercase tracking-widest">공지 내용</label>
            ...
            <div className="p-6 bg-[#f7f5f0] border-t border-[#e0dace] flex gap-3">
              <button onClick={() => setIsAddingNotice(false)} className="flex-1 py-3 rounded-xl bg-white border border-[#e0dace] text-[#9a9080] text-sm font-bold hover:bg-[#e0dace] transition-all">취소</button>
              <button onClick={handleAddNotice} className="flex-[2] py-3 rounded-xl bg-[#4a72a0] text-white text-sm font-bold hover:opacity-90 shadow-lg shadow-[#4a72a0]/20 transition-all">등록하기</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isAddingDisclosure && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-[#e0dace] flex items-center justify-between bg-[#f7f5f0]">
              <h3 className="text-lg font-black text-[#2a2820]">신규 공시 등록</h3>
            ...
            <div className="p-8 space-y-6">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#9a9080] uppercase tracking-widest">공시 대상 자산</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#9a9080] uppercase tracking-widest">공시 유형</label>
                <div className="flex gap-2 p-1 bg-[#e0dace] rounded-xl">
                  {['배당','일반'].map(type => (
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#9a9080] uppercase tracking-widest">공시 제목</label>
                <input type="text" value={editForm.title} onChange={e => setEditForm({ ...editForm, title: e.target.value })}
                  placeholder="공시 제목을 입력하세요"
                  className="w-full bg-[#f7f5f0] border border-[#e0dace] rounded-xl px-4 py-3 text-sm text-[#2a2820] outline-none focus:border-[#4a72a0] font-bold" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-[#9a9080] uppercase tracking-widest">첨부 파일 (PDF URL)</label>
            ...
            <div className="p-6 bg-[#f7f5f0] border-t border-[#e0dace] flex gap-3">
              <button onClick={() => setIsAddingDisclosure(false)} className="flex-1 py-3 rounded-xl bg-white border border-[#e0dace] text-[#9a9080] text-sm font-bold hover:bg-[#e0dace] transition-all">취소</button>
              <button onClick={handleAddDisclosure} className="flex-[2] py-3 rounded-xl bg-[#4a72a0] text-white text-sm font-bold hover:opacity-90 shadow-lg shadow-[#4a72a0]/20 transition-all">등록하기</button>
            </div>
          </div>
        </div>
      )}
